...
class gui(wx.Dialog):
    ...
    def __init__(self, parent, id, title):
        ...
        label3 = wx.StaticText(self, label='Certificate Naming Override:')
        label3.SetPosition((30,190))
        vbox3.Add(label3, 0, wx.ALL, 5)
        ...
        self.text_ctrl = wx.TextCtrl(self, 5, pos=(30,210))
        ...
        self.text_ctrl.SetInitialSize((400, -1))
        ...
        vbox3.Add(self.text_ctrl, 0, wx.ALL, 5)

        # Set the text color to gray and the initial text to temporary text
        self.text_ctrl.SetForegroundColour(wx.Colour(128, 128, 128))
        self.text_ctrl.SetValue('Enter Name Override if applicable')
        ...
        self.text_ctrl.Bind(wx.EVT_SET_FOCUS, self.on_text_ctrl_focus, id=5)
    ...
    # Define the on_text_ctrl_focus() method to clear the temporary text
    def on_text_ctrl_focus(self, event):
        if self.text_ctrl.GetValue() == 'Enter Name Override if applicable':
            self.text_ctrl.SetForegroundColour(wx.BLACK)
            self.text_ctrl.SetValue('')
        event.Skip()  # Skip the event to allow the focus to be set on the text box
